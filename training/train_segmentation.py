...
from datasets.xray_dataset import XraySegmentationDataset
...
class SegmentationTrainer:
    ...
    def _build_dataloaders(self):
        ...
        val_transform = SegmentationAugmentation(
            image_size=tuple(self.config['data']['image_size']),
            is_training=False
        )
        ...
        val_dataset = XraySegmentationDataset(
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, (images, masks) in enumerate(pbar):
            ...
            with torch.no_grad():
                ...
                metrics = self.metrics_calculator.compute(logits, masks)
    ...
    def train(self):
        ...
        for epoch in range(self.config['training']['epochs']):
            ...
            print(f"Train Loss: {train_metrics['train_loss']:.4f}, Train Dice: {train_metrics['train_dice']:.4f}")
            print(f"Val Loss: {val_metrics['val_loss']:.4f}, Val Dice: {val_metrics['val_dice']:.4f}, Val IoU: {val_metrics['val_iou']:.4f}")
...
def main():
    ...
    # Load config
    config_path = Path(__file__).parent.parent / 'configs' / 'segmentation.yaml'
